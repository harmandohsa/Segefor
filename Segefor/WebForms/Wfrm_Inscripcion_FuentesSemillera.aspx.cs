...
{
    public partial class Wfrm_Inscripcion_FuentesSemillera : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            CboCategoriaSF.SelectedIndexChanged += CboCategoriaSF_SelectedIndexChanged;
            ...
            BtnAddEspecieArb.ServerClick += BtnAddEspecieArb_ServerClick;
            GrdEspeciesArb.NeedDataSource += GrdEspeciesArb_NeedDataSource;
            GrdEspeciesArb.ItemCommand += GrdEspeciesArb_ItemCommand;
            ...
            {
                ClUsuario.Insertar_Ingreso_Pagina(39, Convert.ToInt32(Session["UsuarioId"]), ClUsuario.CorrId_IngresoPagina());
                ...
                DataSet dsPermisos = ClUsuario.Get_Roles(Convert.ToInt32(Session["UsuarioId"]), 39);
                ...
                ClUtilitarios.LlenaCombo(ClCatalogos.Listado_CategoriaFS(), CboCategoriaSF, "CategoriaFSId", "CategoriaSF");
                ClUtilitarios.AgregarSeleccioneCombo(CboCategoriaSF, "Sub Categoría");
                ClUtilitarios.LlenaCombo(ClCatalogos.ListadoProcedencia(6), CboProcedencia, "ProcedenciaId", "Procedencia");
                ...
                ClUtilitarios.LlenaCombo(ClCatalogos.Listado_Especie(), CboEspecieArb, "EspecieId", "Especie");
                ClUtilitarios.AgregarSeleccioneCombo(CboEspecieArb, "Especie");
        ...
        void GrdEspeciesArb_ItemCommand(object sender, GridCommandEventArgs e)
        {
            if (e.CommandName == "CmdDel")
            {
                EliminarEspecieArb(Convert.ToInt32(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["EspecieId"].ToString()));
            }
        }

        void GrdEspeciesArb_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            if (dvArb.Count > 0)
                ClUtilitarios.LlenaGridDataView(dvArb, GrdEspeciesArb, "Dt_EspecieArb");
        }

        void BtnAddEspecieArb_ServerClick(object sender, EventArgs e)
        {
            if (ValidaEspecieArb() == true)
            {
                if (ExisteEspecieArb(Convert.ToInt32(CboEspecieArb.SelectedValue)))
                {
                    DivErrEspecieArb.Visible = true;
                    LblErrEspecieArb.Text = "Especie ya existe";

                }
                else
                {
                    CargarGridEspecieArb();
                    AgregaEspecieArb();
                    dvArb = Ds_Temporal.Tables["Dt_EspecieArb"].DefaultView;
                    GrdEspeciesArb.Rebind();
                    //LimpiarEspecie();
                }
        ...
        void CboCategoriaSF_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            ClUtilitarios.LlenaCombo(ClCatalogos.ListadoSubCategoriasRegistro(6, Convert.ToInt32(Session["PersonaId"]), Convert.ToInt32(CboCategoriaSF.SelectedValue)), CboTipoPlantacion, "SubCategoriaId", "Nombre_Subcategoria");
            if ((Convert.ToInt32(CboCategoriaSF.SelectedValue) == 1) || (Convert.ToInt32(CboCategoriaSF.SelectedValue) == 2))
            {
                ClUtilitarios.AgregarSeleccioneCombo(CboTipoPlantacion, "Tipo de Plantación");
                CboTipoPlantacion.Enabled = true;
                DivDatosInv.Visible = false;
            }
            else
            {
                CboTipoPlantacion.Enabled = false;
                DivDatosInv.Visible = true;
                LblValor.InnerText = "Area Total (Ha)";
            }
        }

        void CboTipoPlantacion_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            TxtNombreSistema.Text = CboCategoriaSF.Text + " " + CboTipoPlantacion.Text;
        ...
        void EliminarEspecie(int EspecieId)
        {
            ...
            {
                if (EspecieId == Convert.ToInt32(GrdInventario.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"]))
                {
                ...
                else
                {
                    ...
                    rowNew["CntArboles"] = GrdInventario.Items[i].OwnerTableView.DataKeyValues[i]["CntArboles"];
                    ...
                    rowNew["Distancia"] = GrdInventario.Items[i].OwnerTableView.DataKeyValues[i]["Distancia"];
        ...
        void EliminarEspecieArb(int EspecieId)
        {
            for (int i = 0; i < GrdEspeciesArb.Items.Count; i++)
            {
                if (EspecieId == Convert.ToInt32(GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"]))
                {
                ...
                else
                {
                    ...
                    rowNew["EspecieId"] = GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"];
                    rowNew["Especie"] = GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["Especie"];
        ...
        bool ValidaEspecieArb()
        {
            LblErrEspecieArb.Text = "";
            DivErrEspecieArb.Visible = false;
            bool HayError = false;
            if ((CboEspecieArb.Text == "") || (CboEspecieArb.SelectedValue == ""))
            {
                if (LblErrEspecieArb.Text == "")
                    LblErrEspecieArb.Text = LblErrEspecieArb.Text + "Debe seleccionar la especie";
                else
                    LblErrEspecieArb.Text = LblErrEspecieArb.Text + ", debe seleccionar la especie";
                HayError = true;
            }
        ...
        bool ValidaEspecie()
        {
            ...
            if ((TxtCantArboles.Text == "") || (TxtCantArboles.Text == "0"))
            {
                if (LblMensajeEspecie.Text == "")
                    LblMensajeEspecie.Text = LblMensajeEspecie.Text + "Debe ingresar la cantidad de arboles";
                else
                    LblMensajeEspecie.Text = LblMensajeEspecie.Text + ", debe ingresar la cantidad de arboles";
                HayError = true;
            }
        ...
        bool ExisteEspecie(int EspecieId)
        {
            ...
            for (int i = 0; i < GrdInventario.Items.Count; i++)
            {
                if (Convert.ToInt32(GrdInventario.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"]) == EspecieId)
                {
                    Existe = true;
                    break;
                }
        ...
        bool ExisteEspecieArb(int EspecieId)
        {
            bool Existe = false;
            for (int i = 0; i < GrdEspeciesArb.Items.Count; i++)
            {
                if (Convert.ToInt32(GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"]) == EspecieId)
                {
                    Existe = true;
                    break;
                }
        ...
        void btnAddEspecie_ServerClick(object sender, EventArgs e)
        {
            if (ValidaEspecie() == true)
            {
                ...
                {
                    DivErrEspecie.Visible = true;
                    LblMensajeEspecie.Text = "Especie ya existe";
        ...
        void BtnYes_Click(object sender, EventArgs e)
        {
            ...
            if (TxtTelefonoNotifica.Text != "")
                ...
                XmlDocument iInformacionEspeciesArb = ClXml.CrearDocumentoXML("Especies");
                XmlNode iElementoEspeciesArb = iInformacionEspeciesArb.CreateElement("Arboreas");

                for (int i = 0; i < GrdEspeciesArb.Items.Count; i++)
                {
                    XmlNode iElementoDetalleEspecie = iInformacionEspeciesArb.CreateElement("Item");
                    ClXml.AgregarAtributo("EspecieId", GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["EspecieId"], iElementoDetalleEspecie);
                    iElementoEspeciesArb.AppendChild(iElementoDetalleEspecie);
                }
                iInformacionEspeciesArb.ChildNodes[1].AppendChild(iElementoEspeciesArb);

            ClGestionRegistro.Insertar_Gestion_Fuente_Semillera(GestionId, Convert.ToInt32(CboTipoPlantacion.SelectedValue), 6,Convert.ToInt32(CboProcedencia.SelectedValue),Convert.ToInt32(CboTipoBosque.SelectedValue),iInformacionEspeciesArb, TxtDireccionNotifica.Text, Convert.ToInt32(CboMunicipio.SelectedValue), Telefono, Convert.ToInt32(TxtCelularNotifica.Text.Replace("-", "")), TxtCorreoNotifica.Text, TxtObservaciones.Text, TxtNomFirma.Text, Convert.ToInt32(Session["UsuarioId"]),Convert.ToInt32(CboCategoriaSF.SelectedValue));
            ...
            if (GrdInventario.Items.Count > 0)
            {
                ...
                for (int i = 0; i < GrdInventario.Items.Count; i++)
                {
                    ...
                    ClXml.AgregarAtributo("CntArboles", GrdInventario.Items[i].OwnerTableView.DataKeyValues[i]["CntArboles"], iElementoDetalle);
                    iElementos.AppendChild(iElementoDetalle);
                ...
                ClGestionRegistro.Insert_Inventario_Forestal(GestionId, iInformacion, Convert.ToInt32(Session["UsuarioId"]), TxtNombreSistema.Text);
        ...
        void VistaPrevia()
        {
            ...
            row["CategoriaFS"] = CboCategoriaSF.Text;
            row["TipoPlantacion"] = CboTipoPlantacion.Text;
            
            for (int i = 0; i < GrdEspeciesArb.Items.Count; i++)
            {
                if (Especies == "")
                    Especies = GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["Especie"].ToString();
                else
                    Especies = Especies + ", " + GrdEspeciesArb.Items[i].OwnerTableView.DataKeyValues[i]["Especie"].ToString();
            }
            row["EspeciesArb"] = Especies;
            ...
            row["TotalFuente"] = ClGestionRegistro.Get_Sum_Area_Fuente(Convert.ToInt32(Session["UsuarioId"]), 1);
            ...
            Session["Ds_Formulario_FuenteSemillera"] = Ds_Formulario_FuenteSemillera;
            RadWindow1.Title = "Vista Previa Insripción";
            RadWindow1.NavigateUrl = "~/WeForms_Reportes/Wfrm_RepFormularioFuenteSemillera.aspx?appel=" + HttpUtility.UrlEncode(ClUtilitarios.Encrypt("0", true)) + "";
        ...
        bool Valida()
        {
            ...
            if ((CboCategoriaSF.Text == "") || (CboCategoriaSF.SelectedValue == ""))
            {
                if (LblMensaje.Text == "")
            ...
            if (GrdEspeciesArb.Items.Count == 0)
            {
                if (LblMensaje.Text == "")
                    LblMensaje.Text = LblMensaje.Text + "Debe ingresar el menos una especie arborea";
                else
                    LblMensaje.Text = LblMensaje.Text + ", debe ingresar el menos una especie arborea";
                HayError = true;
            }
            ...
            {
                if (LblMensaje.Text == "")
                    LblMensaje.Text = LblMensaje.Text + "Debe seleccionar la procedencía";
                else
                    LblMensaje.Text = LblMensaje.Text + ", debe seleccionar la procedencía";
                HayError = true;
            }
            if ((CboTipoBosque.Text == "") || (CboTipoBosque.SelectedValue == ""))
            {
                if (LblMensaje.Text == "")
                    LblMensaje.Text = LblMensaje.Text + "Debe seleccionar el tipo de bosque";
                else
                    LblMensaje.Text = LblMensaje.Text + ", debe seleccionar el tipo de bosque";
                HayError = true;
            }
            ...
            if ((GrdInventario.Items.Count > 0) && (TxtNombreSistema.Text == ""))
            {
                if (LblMensaje.Text == "")
                    LblMensaje.Text = LblMensaje.Text + "Debe ingresar el nombre del Sistema Agroforestal";
                else
                    LblMensaje.Text = LblMensaje.Text + ", debe ingresar el nombre del Sistema Agroforestal";
                HayError = true;
            }
        ...
        void AgregaEspecie()
        {
            ...
            rowNew["CntArboles"] = TxtCantArboles.Text;
        ...
        void AgregaEspecieArb()
        {
            DataRow rowNew = Ds_Temporal.Tables["Dt_EspecieArb"].NewRow();
            rowNew["EspecieId"] = CboEspecieArb.SelectedValue;
            rowNew["Especie"] = CboEspecieArb.Text;
            Ds_Temporal.Tables["Dt_EspecieArb"].Rows.Add(rowNew);
        }
        ...
        void GrabarAreas()
        {
            ...
            else if ((TxtAreaFuente.Text == "") || (TxtAreaFuente.Text == "0"))
            {
                DivErrAreas.Visible = true;
                LblErrAreas.Text = "Debe Ingresar el área de la fuente";
            }
            else if ((TxtAreaForestal.Text != "") && (Convert.ToDouble(TxtAreaFuente.Text) > Convert.ToDouble(TxtAreaForestal.Text)))
            {
                DivErrAreas.Visible = true;
                LblErrAreas.Text = "El Área de la fuente no puede ser mayor a el área de forestal";
            }
            else
            {
                ...
                ClGestionRegistro.Insertar_AreasInmuebleRegistro(Convert.ToInt32(Session["UsuarioId"]), Convert.ToInt32(TxtInmuebleId.Text), Convert.ToDouble(ClUtilitarios.IIf(TxtAreaForestal.Text == "", 0, TxtAreaForestal.Text)), Convert.ToDouble(ClUtilitarios.IIf(TxtAreaFuente.Text == "", 0, TxtAreaFuente.Text)));
        ...
        void RetornoAreas()
        {
            ...
            if (dsDatosAreas.Tables["Datos"].Rows.Count > 0)
            {
                ...
                TxtAreaFuente.Text = dsDatosAreas.Tables["Datos"].Rows[0]["AreaFuente"].ToString();
            }
            dsDatosAreas.Clear();
