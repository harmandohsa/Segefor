...
{
    public partial class Wfrm_RepFormularioEmpresas : System.Web.UI.Page
    {
        ...
        Rpt_FormularioEmpresas Reporte = new Rpt_FormularioEmpresas();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataSet Ds_Formulario_Emmpresas = new DataSet();
            Ds_Formulario_Emmpresas = ((DataSet)Session["Ds_Empresas"]);
            Reporte.PrintOptions.PaperOrientation = PaperOrientation.Portrait;
            Reporte.SetDataSource((DataSet)Ds_Formulario_Emmpresas);

            string llamada = ClUtilitarios.Decrypt(HttpUtility.UrlDecode(Request.QueryString["appel"].ToString()), true);
            string NomReporte;
            if (llamada == "1")
                NomReporte = Ds_Formulario_Emmpresas.Tables["Ds_Empresas"].Rows[0]["NUG"] + ".pdf";
            else
            ...
            //if (llamada == "1")
            //{
            //    string Mensaje = "Su solicitud fue enviada exitosamente, con Numero Único de Gestión (NUG): " + Ds_Formulario_Emmpresas.Tables["Ds_Empresas"].Rows[0]["NUG"] + ". Por lo que solicitamos presentarse a la oficina Subregional " + Ds_Formulario_Emmpresas.Tables["Ds_Empresas"].Rows[0]["SubRegion"] + ", con la documentación que deberá presentar para dar trámite a su solicitud.";
